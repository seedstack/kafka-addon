...
public class ClientsPlugin extends AbstractSeedPlugin {
    ...
    private final Specification<Class<?>> messageConsumerSpec = and(classImplements(MessageConsumer.class),
            classAnnotatedWith(Consumer.class));
    ...
    public InitState initialize(InitContext initContext) {
        if (KAFKA_OPTIONAL.isPresent()) {
            ...
            Collection<Class<?>> consumerRebalancerListenerClasses = initContext.scannedTypesBySpecification()
                    .get(consumerRebalancerListenerSpecs);
            if (consumerRebalancerListenerClasses != null) {
                consumerRebalancerListenerClasses.forEach(consumerRebalancerListenerClass ->
                        consumerRebalancerListenerClassesMap
                                .put(consumerRebalancerListenerClass.getAnnotation(Consumer.class).value(),
                                        consumerRebalancerListenerClass));
            }
            Collection<Class<?>> messageConsumerClasses = initContext.scannedTypesBySpecification()
                    .get(messageConsumerSpec);
            messageConsumerClasses.forEach(messageConsumerClass -> initConsumer(
    ...
    private void initConsumer(Class<MessageConsumer> messageConsumerClass,
            Map<String, KafkaConfig.ConsumerConfig> consumerConfigs) {
        ...
        if (consumerConfigs.containsKey(messageConsumerName)) {
            ...
            MessageConsumer messageConsumer = new MessageConsumerAdapter(Key.get(MessageConsumer.class,
                    Names.named(messageConsumerName)), consumerConfig.getPoolConfig());
            messageConsumers.add(messageConsumer);
            pollers.add(new MessageConsumerPoller(consumerConfig, messageConsumer, messageConsumerName));

        } else {
            throw SeedException.createNew(KafkaErrorCode.CONFIG_NOT_FOUND_FOR_MESSAGE_CONSUMER)
    ...
    public Object nativeUnitModule() {
        return new ClientsModule(pollers,
                messageConsumers,
                messageConsumerClasses,
                consumerRebalancerListenerClassesMap,
                producerConfigs);
    }
